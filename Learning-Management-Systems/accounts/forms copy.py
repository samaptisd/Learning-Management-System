...
class StudentAddForm(UserCreationForm):
    ...
    def save(self):
        ...
        student = Student.objects.create(
            ...
            department=self.cleaned_data.get('course')
        )
        student.save()
        return user
...
def validate_file_size(value):
    max_size_kb = 3 * 1024  # Set maximum file size to 3MB
    if value.size > max_size_kb:
        raise forms.ValidationError("The maximum file size that can be uploaded is 3MB.")
...
class ProfileUpdateForm(UserChangeForm):
    ...
    class Meta:
        model = User
        fields = ['reporting_to','email', 'phone', 'zone', 'branch', 'department', 'course_program', 'date_of_join_in_aludecor', 'picture', 'first_name', 'last_name']
